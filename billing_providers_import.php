<?php
	...
	if ($action == 'import' && !empty($_POST['csv_import']) && !empty($_POST['field_map'])) {
		...
		foreach ($lines as $i => $line) {
			...
			if (empty($data['provider_name'])) {
				$errors[] = "Row ".($i + 1).": provider_name is required";
				continue;
			}

			$x = 0;
			$array['providers'][$x]['provider_uuid'] = uuid();
			$array['providers'][$x]['domain_uuid'] = $_SESSION['domain_uuid'];
			$array['providers'][$x]['provider_name'] = $data['provider_name'];
			$array['providers'][$x]['provider_description'] = $data['provider_description'] ?? '';
			$array['providers'][$x]['provider_enabled'] = $data['provider_enabled'] ?? 'true';

			$database->uuid($array['providers'][$x]['provider_uuid']);
			$database->save($array);
			$imported++;
		}
	...
	$document['title'] = $text['title-provider_import'];
	...
	if ($action == 'preview' && !empty($_FILES['ulfile']['tmp_name']) && is_uploaded_file($_FILES['ulfile']['tmp_name'])) {
		...
		$field_options = ['', 'provider_name', 'provider_description', 'provider_enabled'];
		...
		echo "	<div class='heading'><b>".$text['title-provider_import']."</b></div>\n";
	...
	echo "Contoh:<br>\n";
	echo "provider_name,provider_description,provider_enabled<br>\n";
	echo "TELKOM,PT Telkom Indonesia,true<br>\n";
	echo "XL AXIATA,PT XL Axiata Tbk,true\n";
	echo "</span>\n";
	echo "</td>\n";
	echo "</tr>\n";
